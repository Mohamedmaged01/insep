...
<section class="bg-gradient-to-br from-navy via-navy-light to-navy-dark py-20 relative overflow-hidden">
    ...
    <div class="container mx-auto px-4 relative z-10 text-center">
        ...
        <p class="text-white/70 text-lg max-w-2xl mx-auto">{{ $isAr ? 'معهد INSEP هو المعهد الرائد في مجال علوم الرياضة في الشرق الأوسط والعالم العربي' : 'INSEP Institute is the leading sports science institute in the Middle East and Arab world' }}</p>
    </div>
</section>
    ...
    <div class="container mx-auto px-4">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
                ...
                <h2 class="text-3xl md:text-4xl font-black text-navy mb-6">{{ $isAr ? 'نحو ريادة علوم الرياضة في المنطقة' : 'Leading Sports Science in the Region' }}</h2>
                <p class="text-gray-600 leading-relaxed mb-6 text-lg">{{ $isAr ? 'تأسس معهد INSEP لعلوم الرياضة بهدف سد الفجوة في مجال التعليم والتدريب الرياضي المتخصص في المنطقة العربية.' : 'INSEP Sports Science Institute was founded to bridge the gap in specialized sports education and training across the Arab region.' }}</p>
                <p class="text-gray-600 leading-relaxed mb-8">{{ $isAr ? 'منذ تأسيسنا، قدمنا أكثر من 5,000 برنامج تدريبي لأكثر من 20,000 متدرب من مختلف أنحاء الشرق الأوسط.' : 'Since our founding, we have delivered over 5,000 training programs to more than 20,000 trainees from across the Middle East.' }}</p>
                <div class="flex flex-wrap gap-4">
                    @foreach($isAr
                        ? ['شهادات معتمدة دولياً', 'مدربون متخصصون', 'محتوى علمي حديث']
                        : ['Internationally Accredited Certificates', 'Specialized Trainers', 'Up-to-date Scientific Content']
                    as $item)
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            @foreach([
                [
                    ...
                    'desc_ar'  => 'أن نكون المعهد الرائد والمرجع الأول في علوم الرياضة على مستوى الشرق الأوسط والعالم العربي، ونساهم في بناء منظومة رياضية متكاملة تعتمد على العلم والبحث.',
                    'desc_en'  => 'To be the leading and primary reference in sports science across the Middle East and Arab world, contributing to building an integrated sports system based on science and research.',
                    'color' => 'from-navy to-navy-light', 'bg' => 'bg-blue-50',
                ],
                [
                    'title_ar' => 'رسالتنا',  'title_en' => 'Our Mission',
                    'desc_ar'  => 'تقديم برامج تدريبية احترافية معتمدة دولياً تساهم في تطوير الكوادر الرياضية وفق أحدث المعايير العلمية، مع توفير بيئة تعليمية محفزة ومبتكرة.',
                    'desc_en'  => 'Delivering internationally accredited professional training programs that develop sports personnel according to the latest scientific standards, within a motivating and innovative learning environment.',
                    'color' => 'from-red-brand to-red-brand-dark', 'bg' => 'bg-red-50',
                ],
                [
                    'title_ar' => 'أهدافنا',  'title_en' => 'Our Goals',
                    'desc_ar'  => 'بناء جيل متميز من المتخصصين في علوم الرياضة، نشر ثقافة التدريب العلمي، تعزيز الشراكات الدولية، وتطوير البحث العلمي في المجال الرياضي.',
                    'desc_en'  => 'Building a distinguished generation of sports science specialists, spreading scientific training culture, strengthening international partnerships, and advancing sports science research.',
...
<section class="py-20 bg-white">
    <div class="container mx-auto px-4">
        <div class="text-center mb-14">
            <span class="inline-block bg-red-brand/10 text-red-brand px-4 py-1.5 rounded-full text-sm font-bold mb-4">{{ $isAr ? 'فريقنا' : 'Our Team' }}</span>
            <h2 class="text-3xl md:text-4xl font-black text-navy mb-4">{{ $isAr ? 'نخبة من الخبراء والمتخصصين' : 'A Team of Experts & Specialists' }}</h2>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            @foreach([
                ['name_ar' => 'د. أحمد المحمد', 'name_en' => 'Dr. Ahmad Al-Mohammad', 'role_ar' => 'المدير التنفيذي',  'role_en' => 'Executive Director',    'specialty_ar' => 'إدارة رياضية',     'specialty_en' => 'Sports Management',   'rating' => 5.0],
                ['name_ar' => 'د. سارة الأحمد', 'name_en' => 'Dr. Sara Al-Ahmad',     'role_ar' => 'مدرب أول',        'role_en' => 'Senior Trainer',         'specialty_ar' => 'التدريب الرياضي',   'specialty_en' => 'Sports Coaching',     'rating' => 4.9],
                ['name_ar' => 'د. عمر الخالد',  'name_en' => 'Dr. Omar Al-Khaled',    'role_ar' => 'مدرب أول',        'role_en' => 'Senior Trainer',         'specialty_ar' => 'التغذية الرياضية', 'specialty_en' => 'Sports Nutrition',   'rating' => 4.7],
                ['name_ar' => 'د. ليلى المنصور','name_en' => 'Dr. Layla Al-Mansour',  'role_ar' => 'مدربة',           'role_en' => 'Trainer',                'specialty_ar' => 'العلاج الطبيعي',   'specialty_en' => 'Physiotherapy',       'rating' => 4.8],
            ] as $member)
            <div class="bg-white rounded-2xl p-6 border border-gray-100 card-hover text-center group">
                <div class="w-24 h-24 mx-auto mb-5 bg-gradient-to-br from-navy to-navy-light rounded-2xl flex items-center justify-center text-white font-black text-3xl shadow-xl group-hover:scale-105 transform transition-transform duration-500">
                    {{ mb_substr($member['name_ar'], 2, 1) }}
                </div>
                <h3 class="text-lg font-bold text-navy mb-1">{{ $isAr ? $member['name_ar'] : $member['name_en'] }}</h3>
                <p class="text-red-brand font-semibold text-sm mb-1">{{ $isAr ? $member['role_ar'] : $member['role_en'] }}</p>
                <p class="text-gray-500 text-sm mb-3">{{ $isAr ? $member['specialty_ar'] : $member['specialty_en'] }}</p>
...
<section class="bg-gradient-to-br from-red-brand to-red-brand-dark py-16 relative overflow-hidden">
    ...
    <div class="container mx-auto px-4 relative z-10 text-center">
        ...
        <p class="text-white/80 mb-8 max-w-xl mx-auto">{{ $isAr ? 'سجل الآن وابدأ رحلتك المهنية في عالم علوم الرياضة' : 'Register now and start your professional journey in the world of sports science' }}</p>
