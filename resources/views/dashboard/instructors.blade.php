...
<div x-data="instructorsManager()">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-black text-navy">{{ $isAr ? 'إدارة المدربين' : 'Manage Trainers' }}</h1>
            <p class="text-gray-500 text-sm">{{ $isAr ? 'إجمالي' : 'Total' }} {{ $instructors->total() }} {{ $isAr ? 'مدرب' : 'trainers' }}</p>
        </div>
    ...
    <div class="bg-white rounded-2xl p-4 mb-6 border border-gray-100">
        <div class="relative">
            ...
            <form method="GET"><input type="text" name="q" value="{{ request('q') }}" placeholder="{{ $isAr ? 'بحث بالاسم أو البريد أو التخصص... واضغط Enter' : 'Search name / email / specialty... press Enter' }}" class="w-full bg-gray-50 border border-gray-200 rounded-xl pr-10 pl-4 py-2.5 text-sm"></form>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-2xl p-6 border border-gray-100 card-hover text-center"
            ...
            <span class="inline-block px-3 py-1 rounded-lg text-xs font-bold {{ ($inst->status ?? 'active') === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                {{ ($inst->status ?? 'active') === 'active' ? ($isAr ? 'نشط' : 'Active') : ($isAr ? 'معلق' : 'Suspended') }}
            </span>
            <div class="flex justify-center gap-3 mt-4">
                ...
                <form method="POST" action="{{ route('dashboard.instructors.destroy', $inst->id) }}" onsubmit="return confirm('{{ $isAr ? 'هل أنت متأكد من حذف هذا المدرب؟' : 'Are you sure you want to delete this trainer?' }}')">
                    @csrf @method('DELETE')
        ...
        @empty
        <div class="col-span-3 text-center py-12 text-gray-400">{{ $isAr ? 'لا يوجد مدربين مسجلين بعد' : 'No trainers registered yet' }}</div>
        @endforelse
    </div>
    ...
    <div x-show="showAddModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition>
        ...
        <div class="bg-white rounded-2xl p-8 w-full max-w-lg relative z-10 shadow-2xl">
            <h2 class="text-xl font-black text-navy mb-6">{{ $isAr ? 'إضافة مدرب جديد' : 'Add New Trainer' }}</h2>
            <form method="POST" action="{{ route('dashboard.instructors.store') }}" class="space-y-4">
                ...
                <div>
                    <label class="text-sm font-bold text-navy mb-2 block">{{ $isAr ? 'التخصص' : 'Specialty' }}</label>
    ...
    <div x-show="showEditModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition>
        ...
        <div class="bg-white rounded-2xl p-8 w-full max-w-lg relative z-10 shadow-2xl">
            <h2 class="text-xl font-black text-navy mb-6">{{ $isAr ? 'تعديل بيانات المدرب' : 'Edit Trainer Details' }}</h2>
