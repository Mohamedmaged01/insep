...
<div x-data="examsManager()">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-black text-navy">{{ $isAr ? 'الاختبارات' : 'Exams' }}</h1>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead><tr class="bg-gray-50 border-b border-gray-100">
                    ...
                    <th class="text-right text-xs font-bold text-gray-500 uppercase px-6 py-4">{{ $isAr ? 'المحاولات' : 'Attempts' }}</th>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-50 hover:bg-gray-50/50">
                        ...
                        <td class="px-6 py-4"><span class="px-3 py-1 rounded-lg text-xs font-bold bg-navy/10 text-navy">{{ $exam->type === 'quiz' ? ($isAr ? 'اختبار قصير' : 'Quiz') : ($exam->type === 'final' ? ($isAr ? 'نهائي' : 'Final') : $exam->type) }}</span></td>
                    ...
                    @empty
                    <tr><td colspan="8" class="text-center py-12 text-gray-400">{{ $isAr ? 'لا يوجد اختبارات بعد' : 'No exams found yet' }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        <div class="bg-white rounded-2xl p-8 w-full max-w-lg relative z-10 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form method="POST" action="{{ route('dashboard.exams.store') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div>
                        <label class="text-sm font-bold text-navy mb-2 block">{{ $isAr ? 'عدد الأسئلة' : 'Questions Count' }}</label>
                    ...
                    <div>
                        <label class="text-sm font-bold text-navy mb-2 block">{{ $isAr ? 'المدة (دقيقة)' : 'Duration (min)' }}</label>
