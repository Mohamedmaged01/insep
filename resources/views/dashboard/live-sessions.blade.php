...
<div x-data="liveSessionsManager()">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-black text-navy">{{ $isAr ? 'البث المباشر' : 'Live Sessions' }}</h1>
            <p class="text-gray-500 text-sm">{{ $isAr ? 'إجمالي' : 'Total' }} {{ $sessions->count() }} {{ $isAr ? 'جلسة' : 'sessions' }}</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead><tr class="bg-gray-50 border-b border-gray-100">
                    ...
                    <th class="text-right text-xs font-bold text-gray-500 uppercase px-6 py-4">{{ $isAr ? 'الموعد' : 'Scheduled At' }}</th>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-50 hover:bg-gray-50/50">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                <form method="POST" action="{{ route('dashboard.live-sessions.destroy', $sess->id) }}" onsubmit="return confirm('{{ $isAr ? 'هل أنت متأكد من حذف هذه الجلسة؟' : 'Are you sure you want to delete this session?' }}')">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="6" class="text-center py-12 text-gray-400">{{ $isAr ? 'لا يوجد جلسات بث بعد' : 'No live sessions found yet' }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        <div class="bg-white rounded-2xl p-8 w-full max-w-lg relative z-10 shadow-2xl">
            ...
            <form method="POST" action="{{ route('dashboard.live-sessions.store') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="text-sm font-bold text-navy mb-2 block">{{ $isAr ? 'الموعد' : 'Date & Time' }}</label>
    ...
    <div x-show="showEditModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition>
        ...
        <div class="bg-white rounded-2xl p-8 w-full max-w-lg relative z-10 shadow-2xl">
            <h2 class="text-xl font-black text-navy mb-6">{{ $isAr ? 'تعديل جلسة البث' : 'Edit Live Session' }}</h2>
