...
<div x-data="resourcesManager()">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-black text-navy">المحاضرات المسجلة</h1>
            <p class="text-gray-500 text-sm">إجمالي {{ $resources->count() }} محتوى</p>
        </div>
        ...
        <div class="bg-white rounded-2xl p-6 border border-gray-100 card-hover">
            <div class="flex justify-between items-start mb-4">
                ...
                <div class="flex gap-2">
                    ...
                    <form method="POST" action="{{ route('dashboard.resources.destroy', $res->id) }}" onsubmit="return confirm('هل أنت متأكد من حذف هذا المحتوى؟')">
                        @csrf @method('DELETE')
            ...
            <div class="flex items-center justify-between">
                <span class="text-xs font-bold px-2 py-1 bg-navy/10 text-navy rounded-lg">{{ $res->type ?? 'فيديو' }}</span>
                @if($res->file_url)
                <a href="{{ $res->file_url }}" target="_blank" class="text-xs text-blue-500 hover:underline">رابط ←</a>
                @endif
            </div>
        </div>
        @empty
    ...
    <div x-show="showAddModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition>
        ...
        <div class="bg-white rounded-2xl p-8 w-full max-w-lg relative z-10 shadow-2xl">
            <h2 class="text-xl font-black text-navy mb-6">إضافة محتوى جديد</h2>
            <form method="POST" action="{{ route('dashboard.resources.store') }}" class="space-y-4">
                @csrf
                <div>
                    <label class="text-sm font-bold text-navy mb-2 block">العنوان</label>
                ...
                <div>
                    ...
                    <select name="type" class="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:border-navy transition-colors">
                        ...
                        <option value="VideoLink">رابط فيديو</option>
                ...
                <div>
                    <label class="text-sm font-bold text-navy mb-2 block">الرابط أو مسار الملف</label>
                ...
                <div>
                    <label class="text-sm font-bold text-navy mb-2 block">الدورة (اختياري)</label>
                ...
                <div>
                    <label class="text-sm font-bold text-navy mb-2 block">المجموعة (اختياري)</label>
    ...
    <div x-show="showEditModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition>
        ...
        <div class="bg-white rounded-2xl p-8 w-full max-w-lg relative z-10 shadow-2xl">
            <h2 class="text-xl font-black text-navy mb-6">تعديل المحتوى</h2>
