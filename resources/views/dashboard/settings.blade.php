...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="bg-white rounded-2xl p-8 border border-gray-100 text-center">
        ...
        <p class="text-sm text-gray-500" style="font-family: 'Roboto', sans-serif">{{ $user->email }}</p>
        <span class="inline-block mt-3 px-4 py-1.5 rounded-xl text-xs font-bold bg-navy/10 text-navy">{{ $user->role === 'admin' ? 'مدير النظام' : ($user->role === 'student' ? 'طالب' : 'مدرب') }}</span>
    </div>
        ...
        <form class="space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" value="{{ $user->name }}" class="w-full border-2 border-gray-200 rounded-xl px-4 py-3.5">
                </div>
                <div>
                    <label class="text-sm font-bold text-navy mb-2 block">البريد الإلكتروني</label>
                    <input type="email" value="{{ $user->email }}" class="w-full border-2 border-gray-200 rounded-xl px-4 py-3.5" dir="ltr">
                </div>
            </div>
            <div>
                <label class="text-sm font-bold text-navy mb-2 block">رقم الجوال</label>
                <input type="tel" value="{{ $user->phone ?? '' }}" class="w-full border-2 border-gray-200 rounded-xl px-4 py-3.5" dir="ltr">
            </div>
            <div>
