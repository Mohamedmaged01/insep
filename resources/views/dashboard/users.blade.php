...
<div class="p-6" x-data="{ showAdd: false, editUser: null, resetUser: null }">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="relative" x-data="{ show: false }">
                            <input :type="show ? 'text' : 'password'" name="password" required dir="ltr" class="w-full border-2 border-gray-200 rounded-xl pr-4 pl-11 py-2.5 focus:border-navy transition-colors text-sm">
                            <button type="button" @click="show = !show" tabindex="-1"
                                class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-navy transition-colors"
                                :title="show ? '{{ $isAr ? 'إخفاء' : 'Hide' }}' : '{{ $isAr ? 'إظهار' : 'Show' }}'">
    ...
    {{-- Reset Password Modal (admin + super_admin) --}}
    @if(auth()->user()->isAdminOrAbove())
            ...
            <template x-if="resetUser">
                <form method="POST" :action="`/dashboard/users/${resetUser.id}/reset-password`" class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative" x-data="{ show: false }">
                            <input :type="show ? 'text' : 'password'" name="password" required minlength="6" dir="ltr"
                                class="w-full border-2 border-gray-200 rounded-xl pr-4 pl-11 py-2.5 focus:border-yellow-400 transition-colors text-sm"
