        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50/50 transition">
                    ...
                    <td class="px-5 py-3">
                        @php $typeColors = ['auto'=>'bg-blue-100 text-blue-700','manual'=>'bg-purple-100 text-purple-700','bulk'=>'bg-teal-100 text-teal-700']; @endphp
                        <span class="px-2 py-0.5 rounded-lg text-xs font-semibold {{ $typeColors[$cert->type ?? 'manual'] ?? 'bg-gray-100 text-gray-600' }}">
                            {{ ['auto'=>($isAr?'تلقائي':'Auto'),'manual'=>($isAr?'يدوي':'Manual'),'bulk'=>($isAr?'جماعي':'Bulk')][$cert->type ?? 'manual'] ?? $cert->type }}
                        </span>
                    </td>
